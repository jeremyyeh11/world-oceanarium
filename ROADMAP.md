...
- [ ] Jeremy feel review (dev_10): mahi keeps waving its tail and banks through turns (no C-curl freeze); pitch relaxes to level when hovering; mako/mahi arc wide at swim speed; more vertical spread with periodic off-frame excursions. Tune `turnRadiusBodyLengths`, per-species `boundsYMin`, `turnTriggerThreshold`, and curve-deform `turnIntentScale`/`strength` if needed.
