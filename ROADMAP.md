...
- [ ] Jeremy feel review (dev_11): mahi straightens its tail as it exits a turn, then swims on (no one-sided wave against a locked bend); mahi banks through turns without a C-curl; pitch relaxes when hovering; mako/mahi arc wide at swim speed; vertical spread with periodic off-frame excursions. Tune `straightenFloor`, `turnIntentScale`/`strength`, `turnTriggerThreshold`, `turnRadiusBodyLengths`, `boundsYMin` as needed.
