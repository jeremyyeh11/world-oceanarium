...
- [ ] Jeremy feel review (dev_12): mahi tail no longer spins during turns; it swims continuously and banks through turns; straightens on turn-exit; pitch relaxes when hovering; mako/mahi arc wide at swim speed; vertical spread with periodic off-frame excursions. Tune `straightenFloor`, curve-deform `turnIntentScale`/`strength`, `turnRadiusBodyLengths`, `boundsYMin` as needed. (mahi `snap_left`/`snap_right` clips + `turnTriggerThreshold` are now unused for turns — could be repurposed for sharp evasive turns later.)
