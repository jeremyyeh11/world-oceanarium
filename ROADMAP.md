...
- [ ] Jeremy feel review (dev_13): mahi banks in a gentle curve through turns (no C-curl, no spin); swims continuously; straightens on turn-exit; pitch relaxes when hovering; wide arcs at swim speed; vertical spread. If the mahi bank now reads too subtle/stiff, raise `turnIntentScale`/`maxAngleDegrees` slightly; if any curl remains, lower `turnIntentScale` toward 0 (matching the mako, which sets no turn-intent bend). (mahi `snap_left`/`snap_right` clips are unused for turns — repurpose for sharp evasive turns later if wanted.)
